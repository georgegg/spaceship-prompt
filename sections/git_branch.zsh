...
SPACESHIP_GIT_BRANCH_COMMIT_SYMBOL="${SPACESHIP_GIT_BRANCH_COMMIT_SYMBOL=" -o- "}"
...
SPACESHIP_GIT_BRANCH_TAG_SYMBOL="${SPACESHIP_GIT_BRANCH_TAG_SYMBOL=" 🏷  "}"
...
spaceship_git_branch() {
  ...
  if [[ $SPACESHIP_GIT_BRANCH_SHOW_TAG == true ]]; then
    local git_tag_name=$(command git describe --tags --exact-match $git_commit_sha 2>/dev/null)
